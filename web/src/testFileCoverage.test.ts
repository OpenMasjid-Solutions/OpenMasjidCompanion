 ...
 * them covering a shipped bug — before anyone noticed. The server half of this repo runs the
 * identical check against its own list.)
 *
 * If this fails: add the named file to the `test` script in web/package.json.
 */
...
// ESM: no __dirname. This file is web/src/, so the package root is one up.
...
function testFilesOnDisk(dir: string): string[] {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      out.push(path.relative(WEB_DIR, full).split(path.sep).join('/'));
    }
  }
  return out;
}
...
function testScript(): string {
  const pkg = JSON.parse(fs.readFileSync(path.join(WEB_DIR, 'package.json'), 'utf8')) as { scripts: { test: string } };
...
test('every test file on disk is one npm test actually runs', () => {
  ...
  const missing = testFilesOnDisk(path.join(WEB_DIR, 'src')).filter((f) => !listed.has(f));
  assert.deepEqual(
    missing,
    [],
...
test('the test script does not name a file that no longer exists', () => {
  ...
  const gone = named.filter((f) => !fs.existsSync(path.join(WEB_DIR, f)));
